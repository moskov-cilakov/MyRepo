/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
} from "class-validator";
import { ServiceAgentUpdateManyWithoutOfficesInput } from "./ServiceAgentUpdateManyWithoutOfficesInput";
import { Type } from "class-transformer";
import { UserUpdateManyWithoutOfficesInput } from "./UserUpdateManyWithoutOfficesInput";

@InputType()
class OfficeUpdateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  officeName?: string | null;

  @ApiProperty({
    required: false,
    type: () => ServiceAgentUpdateManyWithoutOfficesInput,
  })
  @ValidateNested()
  @Type(() => ServiceAgentUpdateManyWithoutOfficesInput)
  @IsOptional()
  @Field(() => ServiceAgentUpdateManyWithoutOfficesInput, {
    nullable: true,
  })
  serviceAgents?: ServiceAgentUpdateManyWithoutOfficesInput;

  @ApiProperty({
    required: false,
    type: () => UserUpdateManyWithoutOfficesInput,
  })
  @ValidateNested()
  @Type(() => UserUpdateManyWithoutOfficesInput)
  @IsOptional()
  @Field(() => UserUpdateManyWithoutOfficesInput, {
    nullable: true,
  })
  users?: UserUpdateManyWithoutOfficesInput;
}

export { OfficeUpdateInput as OfficeUpdateInput };
